...
rem === Warning Dialog (headless 模式跳过) ===
if not defined HEADLESS powershell -Command "Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('程序运行期间请不要操作电脑。', '提示', 'OK', 'information')"
...
rem === Display Desktop (headless 模式跳过, 避免最小化主程序窗口) ===
...
@REM rem === Zip output folder (headless 模式跳过压缩和上传) ===
if not defined HEADLESS (
    @REM echo 打包输出文件...
...
echo ======== 配置导出完成 ========
if not defined HEADLESS powershell -Command "Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('配置导出已完成。', '完成', 'OK', 'Information')"
